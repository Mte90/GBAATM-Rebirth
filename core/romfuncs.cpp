...
int patchrom(char * gbaromname,char * newgbaromname,unsigned int * mycheatint, int cheatintlen,int freeram, SLOMOSTRUCT slomostruct,ENABLEDISABLESTRUCT edstruct,int excycles,char * path,int wantmenu,unsigned int * menuint, int cheatselectram, int vblankcheck, unsigned int * temptrainermenuint) {
    ...
    while (cheatptr[0]<strlen(cheatcodes)) {
        ...
        //	* FAAAAAAA YYYY - if 16-bit value at address AND YYYY = 0 then skip next code
                ...
                *decryptval=decval;
                ...
                if (superon>0) {
                    superon-=6;
                    sprintf(asmtop+strlen(asmtop),"0x%X, 0x%X, 0x%X",byteflip((tempdec>>16)&0xffff)&0xffff,byteflip(tempdec&0xffff)&0xffff,byteflip(decval)&0xffff);
                    if (superon>0) {
